...
@WebServlet("/DestinationChangeController")
public class DestinationChangeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
     ...
     // DestinationDAO 객체 생성
        ...
        // 회원 수정 결과 확인
        boolean updateCheck = dDao.destinationUpdateManager(dDto);
        
     // 추가된 로깅 부분
        System.out.println("destination Update Result: " + updateCheck);
        
        if (updateCheck) {
            // 회원 수정 성공 시, 다시 MemberList.jsp로 이동
            ArrayList<DestinationDTO> destinationList;
			try {
				destinationList = dDao.selectDestinationList();
